  ...
  data() {
    return {
      ...
      columns: [
        ...
        { name: "description", align: "left", label: "Cliente", field: "description",sortable: true },
         { name: "equipamento", align: "left", label: "Equipamento", field: "equipamento",sortable: true },
...
/*         { name: "user",align: "left", label: "Usuário", field: "user",sortable: true }, */
            ...
            if (this.listOrders.hasOwnProperty(i)) {
              ...
              this.listOrders[i].OrderItems.forEach(c => {
                    ...
                    this.ordem.push({
                          ...
                          user: this.serializeUs(this.listOrders[i].User.name),
 ...
 /* 
                    for (let ix in this.listOrders[i].OrderItems) {
                        if(artI==''){
                          arI = this.listOrders[i].OrderItems[ix].Article.description
                        } else {
                          arI = arI+', '+ this.listOrders[i].OrderItems[ix].Article.description
                        }
                    }

                   arI=''        
                    if(this.listOrders[i].OrderItems !==null){
                        this.listOrders[i].OrderItems.forEach(c => {
                            if(artI==''){
                              arI = c.Article.description
                            } else {
                              arI = arI+', '+ c.Article.description
                            }
                ...
                var dataEx = this.listOrders[i].createdAt.split('T')
                    this.data.push({
                      createdAt: dataEx[0],
                          ...
                          equipamento: this.listOrders[i].Material !== null
