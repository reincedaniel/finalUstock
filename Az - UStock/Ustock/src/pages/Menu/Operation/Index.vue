        ...
        <q-tab-panel name="itens">
          <div class="q-pa-md">
            <div class="row q-gutter-sm">
              ...
              <q-btn size="md"  v-if="!Object.keys(selected).length && (ad==1 || ad==2)" round color="primary" @click="medium = true" icon="add_box" />
              <q-btn
                v-if="Object.keys(selected).length && (ad==1 || ad==2)"
                @click="editOperation()"
                size="md"
                round
                color="warning"
          ...
          for (let i in this.listOperations) {
            if (this.listOperations.hasOwnProperty(i)) {
              this.dataOperation.push({
                ...
                user: this.serializeUs(this.listOperations[i].User.name)
